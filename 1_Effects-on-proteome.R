...
Gender_genes <- res_summary$Master_gene_symbol
Gender_selected_genes <- res_summary[order(res_summary$Gender_pval), "Master_gene_symbol"][1:10]
Gender_genes[!Gender_genes %in% Gender_selected_genes] <- ""
...
ggplot(data = res_summary) +
  geom_point(aes(x = Gender_coef, y = -log10(Gender_pval), 
             ...
             shape = 21, color = "black", stroke = 0.3) +
  geom_text_repel(aes(x = Gender_coef, y = -log10(Gender_pval), label = Gender_genes), size = 3, max.overlaps = 99) +
  scale_size(range = c(0.1, 4)) +
  scale_fill_gradientn(colors = c("blue", "white", "red"), breaks = c(-20, 0, 20), limits = c(-20, 20), oob = scales::squish) +
  theme_classic(base_size = 16) +
  theme(aspect.ratio = 1, legend.position = "none") +
